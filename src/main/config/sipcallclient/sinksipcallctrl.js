...
LASipCallCtrl.prototype.sendInvite = function(fromUser, callback) {
    ...
    self.post('/ws/sipcall/waitForInvite', waitForInviteParams, function(error, data) {
        if (error) {
            self.log("waitForInvite failed, error=" + error);
            callback(error);
            return;
        }
        self.log(data.callId + " waitForInvite succeeded");
        ...
        var timeoutId = setTimeout(function(callId) {
            self.log(callId + " timeout waiting for invite");
